...
function _initialize() {
    ...
    // JBIDE-18900 Basic FeedHenry support (need to use 'disabled' proxy) @see InitRipplePreferences.java overrideProxySettingIfNeeded 
    if (window.csOverridenProxy) {
        select.value = window.csOverridenProxy;
        db.save(PROXY_SETTING, select.value);
        delete window.csOverridenProxy; // removing from scope
    }

    if (select.value === PROXY_SETTINGS_LIST.local) {
        localProxyPortSetting = jQuery("#" + LOCAL_PROXY_PORT_SETTING);
        db.save(LOCAL_PROXY_PORT_SETTING, location.port); // CORDOVASIM: localPort = CordovaSim jetty port which is uneditable
        localProxyPortSetting
